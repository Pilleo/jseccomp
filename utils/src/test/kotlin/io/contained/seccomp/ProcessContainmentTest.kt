...
import io.contained.IsolatedProcessTester
...
class ProcessContainmentTest {
    ...
    fun `installOnProcess applies containment globally`() {
        IsolatedProcessTester.runIsolatedTest("io.contained.seccomp.SeccompIsolatedTestApp", "process-wide")
    }

    @Test
    @EnabledIfLinuxAndSupported
    fun `subsequent threads inherit process-wide filter`() {
        IsolatedProcessTester.runIsolatedTest("io.contained.seccomp.SeccompIsolatedTestApp", "inheritance")
    }

    @Test
    @EnabledIfLinuxAndSupported
    fun `process-wide NO_NETWORK does not break NIO selector`() {
        IsolatedProcessTester.runIsolatedTest("io.contained.seccomp.SeccompIsolatedTestApp", "nio-stability")
    }

    @Test
    @EnabledIfLinuxAndSupported
    fun `thread-local installation respects filter depth`() {
        IsolatedProcessTester.runIsolatedTest("io.contained.seccomp.SeccompIsolatedTestApp", "thread-depth")
    }
}
